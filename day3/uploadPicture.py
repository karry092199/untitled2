...
#有时页面加载完，但是javascript的控件还没有创建好，所以需要加一个time.sleep(),提供程序稳定性
time.sleep(2)
#driver.find_element_by_css_selector("#filePicker  label").click()
#因为真正负责上传文件的页面元素是<input  type="file"...
#所以我们可以直接操作这个控件
#这个空间可以直接输入图片的路径
driver.find_element_by_name("file").send_keys("D:/33.jpg")

#页面太长，点击不了下面的按钮，怎么操作滚动条
#range是区间，默认从0开始，到长度减一结束range(10)表示0到9，一共10个数字
ad=ActionChains(driver)
for i in range (10):
    ad.send_keys(Keys.ARROW_DOWN )
ad.perform()


#点击开始上传，同时用三个class定位
driver.find_element_by_css_selector(".uploadBtn.state-finish.state-ready") .click()
#alert这个控件不是直接弹出来，需要时间等待
time.sleep(3)
